...
/* Tant que NEXT_PUBLIC_SITE_ORIGIN n'est pas posee, on tourne sur le
   domaine de repli. C'est le cas de la demo Vercel : le site doit
   alors rester hors des moteurs, sinon il s'indexe sous une adresse
   qui n'est pas celle de NOA et se met en concurrence avec le vrai
   site le jour de la mise en ligne.

   Poser la variable suffit a tout rouvrir : robots, sitemap,
   canonical et JSON-LD suivent ensemble. */
export const IS_LIVE = Boolean(process.env.NEXT_PUBLIC_SITE_ORIGIN);
...
export const MENU = [
  ...
  /* La carte imprime ces deux lignes dans le même bloc orange que
     ...
     et en bleu : ce sont des vêtements NOA, pas des boissons. */
  { id: 'goodies', label: 'Goodies', accent: 'blue', items: [
